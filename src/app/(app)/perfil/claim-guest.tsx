...
import { Card, ErrorBanner } from "@/components/ui/legacy";
...
export function ClaimGuest({ guests }: { guests: { id: string; name: string; cards: number }[] }) {
  ...
  return (
    <Card className="mt-6 space-y-2">
      ...
      <p className="text-xs text-muted-foreground">Hay historial cargado a nombre de invitados. Si es tuyo, reclamalo y pasa a tu cuenta.</p>
      <ErrorBanner message={error} />
      <ul className="divide-y divide-border">
        {guests.map((g) => (
          <li key={g.id} className="flex items-center justify-between py-2 text-sm">
            <span>
              {g.name} <span className="text-muted-foreground">· {g.cards} tarjetas</span>
            </span>
            <Button
              variant="secondary"
              disabled={pending}
