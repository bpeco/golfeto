...
- [x] **En producción** (2026-09-25, a pedido del dueño): fast-forward de `claude/vigilant-bardeen-6iwzid` de `1924828` a `0b4b668` (rediseño), después `606dc76` y `864f6a7` (foto desde la galería) y `743847e` (la espera de la foto muestra los pasos en vez de un contador de segundos). Cada pase a producción es un fast-forward de esta rama. Sin cambios en la base. Para volver a la versión de antes del rediseño: Instant Rollback en Vercel al deploy `dpl_5JZsaVjb4EYJrHjmbnoWerZbax3b` (commit `1924828`)
...
- [x] Tira de hoyos del modo hoyo con la notación de la tarjeta (2026-09-25, pedido del dueño): cada hoyo con su marca (`ScoreMark xs`: círculo birdie, doble círculo eagle, cuadrado bogey, doble cuadrado doble bogey o peor, barra no terminado, par sin marca); el hoyo actual con recuadro en vez de celda invertida. Validado con la skill UI/UX Pro Max (instalada en `.claude/skills/`): la forma no depende solo del color, contraste de las marcas 4,9–8,2, foco visible, respuesta al tocar; el ancho por hoyo (touch target) quedaba en 36–40 px y ahora la tira va de borde a borde (40–48 px según el teléfono)
