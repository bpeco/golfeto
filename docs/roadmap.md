...
- [x] **En producción** (2026-09-25, a pedido del dueño): fast-forward de `claude/vigilant-bardeen-6iwzid` de `1924828` a `0b4b668`. Sin cambios en la base. Para volver atrás: Instant Rollback en Vercel al deploy `dpl_5JZsaVjb4EYJrHjmbnoWerZbax3b` (commit `1924828`)
- [x] Foto desde la galería (2026-09-25, pedido del dueño): la foto de la tarjeta (Partida) y la de la tarjeta del club (Canchas) se pueden sacar con la cámara o elegir de la galería (`usePhotoPicker`: un input con `capture` y otro sin)
...
### Para probar en el teléfono (preview de Vercel de `claude/sleepy-wozniak-v86mqg`)

- [ ] Puerta 1: `/dev/playground?variant=A|B|C`, confirmar A (o elegir) y borrar B y C de `variants.tsx`
...
- [ ] Foto de la tarjeta: subir, leer, revisar filas, cargar golpes; ver la foto grande
...
- [ ] Instalar Impeccable desde una red libre y correr `/impeccable critique` con el skill completo; re-autorizar el conector de Vercel (403) — ver handoff
